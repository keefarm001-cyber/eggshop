...
async function initSchema() {
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS users (
      ...
      password_hash TEXT NOT NULL, full_name VARCHAR(100) NOT NULL,
      role_id INTEGER REFERENCES roles(id), branch_id INTEGER REFERENCES branches(id),
    ...
    const eggProducts = [
      ['EGG-0','ไข่ไก่เบอร์ 0'],['EGG-1','ไข่ไก่เบอร์ 1'],['EGG-2','ไข่ไก่เบอร์ 2'],
      ['EGG-3','ไข่ไก่เบอร์ 3'],['EGG-4','ไข่ไก่เบอร์ 4'],['EGG-5','ไข่ไก่เบอร์ 5'],
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS stock_receipt_items (
      ...
      qty_tray INTEGER, cost_per_unit NUMERIC(10,4), total_cost NUMERIC(10,2))`);

    await pool.query(`CREATE TABLE IF NOT EXISTS stock_transfers (
      id SERIAL PRIMARY KEY, from_branch_id INTEGER REFERENCES branches(id),
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS shifts (
      ...
      opening_cash NUMERIC(10,2) DEFAULT 0, closing_cash NUMERIC(10,2),
      expected_cash NUMERIC(10,2), cash_difference NUMERIC(10,2),
      status VARCHAR(20) DEFAULT 'open', note TEXT)`);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS sale_items (
      ...
      product_id INTEGER REFERENCES products(id), qty_set INTEGER NOT NULL,
      unit_size INTEGER NOT NULL, qty_unit INTEGER NOT NULL,
      price_per_set NUMERIC(10,2) NOT NULL, subtotal NUMERIC(10,2) NOT NULL)`);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS credit_payments (
      ...
      paid_at TIMESTAMPTZ DEFAULT NOW(), created_by INTEGER REFERENCES users(id))`);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS damage_photos (
      id SERIAL PRIMARY KEY, branch_id INTEGER REFERENCES branches(id),
      product_id INTEGER REFERENCES products(id), photo_url TEXT NOT NULL,
